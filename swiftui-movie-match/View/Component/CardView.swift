...
struct CardView_Previews: PreviewProvider{
  static var previews: some View {
      let movieManager = MovieManager() // Create an instance of MovieManager
      
      if let firstMovie = movieManager.movieList?.first {
          // Safely unwrap the first movie if movieList is not nil
          CardView(movie: firstMovie)
              .previewLayout(.fixed(width: 375, height: 600))
      } else {
          // Handle the case where movieList is nil or empty
